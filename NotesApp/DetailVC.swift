...
//  NotesApp
//
//  Created by Байсангур on 30.01.2023.
//
...
final class DetailVC: UIViewController, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.textView.text = self.note.bodyNote
    ...
    @objc private func saveNote() {
        ...
        self.dataManager.deleteNote(note)
    ...
    @objc private func createAlert() {
        ...
        alert.addTextField { textField in
            if let textTitle = self.note.text {
        ...
        let alertActionCreateNote = UIAlertAction(title: Text.alertActionCreateNoteTitle, style: .default) {_ in
            if let text = alert.textFields?.first?.text,
               alert.textFields?.first?.text != "" {
                ...
                self.note.text = Text.noteTitleNotEntered
                self.navigationItem.title = Text.noteTitleNotEntered
            }
        }
        
        let alertActionCancel = UIAlertAction(title: Text.alertActionCancelTitle, style: .destructive) {_ in
